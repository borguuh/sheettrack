...
// Helper: format date for Sheets; blank if epoch/invalid/missing
function sheetDate(d: unknown): string {
  if (!d) return "";
  const dt = d instanceof Date ? d : new Date(d as any);
  const t = dt.getTime();
  // treat Unix epoch (1970) or invalid dates as blank
  if (Number.isNaN(t) || t === 0 || dt.getUTCFullYear() === 1970) return "";
  // date-only for readability in Sheets
...
if (
  ...
  console.warn(
    "Google Sheets credentials not configured. Google Sheets sync will be disabled."
  );
}
...
class GoogleSheetsService {
          ...
          requestBody: {
            values: [
              [
                ...
                issue.impact,
                issue.status,
                sheetDate(issue.expectedFixDate),
                issue.createdBy || "",
                issue.updatedBy || "",
              ],
            ],
  ...
  async initializeSheet() {
    if (!this.isEnabled) {
      console.log(
        "Google Sheets initialization skipped - credentials not configured"
      );
      return;
    }
